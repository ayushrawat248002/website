  ...
  return (
    ...
    >
      <body className="m-0 p-0 max-[370px]:w-[375px] realtive  min-h-full   w-full bg-white text-black font-[var(--font-poppins)]">
        ...
        {/* 🔝 HEADER */}
        <header className="sticky pr-2  top-0 z-100 w-full   h-[130px] bg-white">
          
          <div className=" relative boc-border h-full w-full items-center justify-between px-10">

            {/* Logo */}
            <h1 className="font-bold absolute top-6 ml-3 text-lg text-gray-900 sm:text-xl">
              Apparel-website
            </h1>
              ...
              {[{img : icon1, text : enabled ? "women" : "men"},{ img : tag , text : "deals"},{img : bag, text : 'All'},{ img : star , text : "For You"}].map((val,i) => (
                ...
                >
                  ...
                  >
                    ...
                    <h2 className="absolute  bottom-0 text-[15px] text-black">
                      {val.text}
                    </h2>
                  </div>
                </div>
        ...
        {/* 📦 MAIN CONTENT */}
        <main className="w-full -translate-y-1">
          {children}
        </main>


     <footer className="sticky z-90 pl-2 pr-2 bg-orange-300 flex justify-between items-center bottom-0 h-[72px] m-0 border-none w-full bg-black">
